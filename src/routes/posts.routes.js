...
router.get("/:idPost", async (req, res) => {
  ...
  try {
    ...
    Validations.idPost(idPost);

    connection = await connectDB();
    const query =
      "SELECT posts.id, posts.title, posts.content, posts.category, tags.name as tag, posts.createdAt, posts.updatedAt FROM posts JOIN posts_tags ON posts.id = posts_tags.idPosts JOIN tags ON posts_tags.idTags = tags.id WHERE posts.id = ?";
    const [post] = await connection.query(query, [idPost]);
...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    if (term) {
      ...
      query =
        "SELECT posts.id, posts.title, posts.content, posts.category, JSON_ARRAYAGG(tags.name) AS tags, posts.createdAt, posts.updatedAt FROM posts JOIN posts_tags ON posts.id = posts_tags.idPosts JOIN tags ON posts_tags.idTags = tags.id GROUP BY posts.id ";
      [posts] = await connection.query(query);
    }
